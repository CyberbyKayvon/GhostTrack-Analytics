...
const HeatmapViewer = ({ siteId }) => {
  ...
  const getPageBreakdown = () => {
    ...
    data.clicks.forEach(click => {
      // Extract page from full URL stored in database
  ...
  return (
    <div className="bg-white rounded-2xl p-4 md:p-6 shadow-lg">
      ...
      {data && (
        <div className="grid grid-cols-3 gap-2 md:gap-4 mb-4 md:mb-6">
          ...
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-purple-200">
            ...
            <p className="text-2xl md:text-3xl font-black text-gray-900">{pageBreakdown.length}</p>
          </div>
        </div>
      )}
      ...
      {pageBreakdown.length > 0 && (
        <div className="mb-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 border-2 border-gray-200">
          ...
          <div className="space-y-2">
            {pageBreakdown.map((page, index) => {
              ...
              const pageName = page.url.split('/').pop() || page.url.replace('https://', '').replace('http://', '');

              return (
                <div key={index} className="bg-white rounded-lg p-3 border border-gray-200 hover:border-orange-300 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <a
                        ...
                        className="text-sm font-semibold text-gray-700 hover:text-orange-600 truncate flex items-center gap-1"
                      >
                        {pageName || 'Homepage'}
        ...
        {loading ? (
          ...
          <div className="relative bg-white mx-auto" style={{ height: '300px', maxHeight: '300px', width: '100%', maxWidth: '100%' }}>
            ...
            <div className="absolute bottom-2 right-2 md:bottom-4 md:right-4 bg-white rounded-lg shadow-lg p-2 md:p-3 border-2 border-gray-200">
              <p className="text-xs font-bold text-gray-700 mb-1 md:mb-2">Click Density</p>
